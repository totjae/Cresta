...
def _snapshot(
    ...
    *,
    ...
    bid: Decimal = Decimal("100.7"),
    last: Decimal = Decimal(101),
) -> None:
    n = _next_seq()
    snap = MarketSnapshot(
        symbol=symbol,
        market="KRX",
        source="TEST",
        sequence_or_hash=f"rg-{symbol}-{n}",
        source_sequence=n,
        payload_hash="a" * 64,
        last_price=last,
        open_price=last,
...
def _decision(db: Session, action: str = "BUY") -> Decision:
    ...
    decision = Decision(
        purpose="TRADING",
        evaluation_request_id=f"rg-eval-{action}-{n}",
...
def _seed(
    db: Session, admin: User, settings: Settings, *, risk_overrides: dict | None = None
) -> Decision:
...
def test_buy_blocked_when_total_exposure_exceeded(
    ...
    pos = Position(
        ...
        managed_quantity=260,
        managed_average_price=Decimal(101),
        state="OPEN",
    )
    db.add(pos)
    db.commit()
...
def test_buy_blocked_when_open_positions_limit(
    ...
    db.add(
        Position(
            ...
            managed_quantity=5,
            managed_average_price=Decimal(60000),
            state="OPEN",
        )
    )
    db.commit()
...
def test_buy_blocked_when_daily_loss_exceeded(db: Session, admin: User, settings: Settings) -> None:
    ...
    db.add(
        Position(
            ...
            managed_quantity=10,
            managed_average_price=Decimal(300),
            state="OPEN",
        )
    )
    db.commit()
...
def test_buy_blocked_when_spread_too_wide(db: Session, admin: User, settings: Settings) -> None:
...
def test_buy_blocked_when_active_daily_loss_event(
    ...
    create_risk_event(
        db,
        scope=RISK_EVENT_SCOPE_DAILY_LOSS,
        rule_code="DAILY_LOSS_LIMIT",
        severity="HIGH",
        account_alias=ACCOUNT_ALIAS,
        input_record={"note": "halt"},
        correlation_id="halt",
        now=NOW,
    )
    db.commit()
