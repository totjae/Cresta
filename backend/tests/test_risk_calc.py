...
def _snapshot(db: Session, symbol: str, last_price: Decimal, *, bid: Decimal | None = None, ask: Decimal | None = None, sequence: int = 1) -> MarketSnapshot:
    snap = MarketSnapshot(
        symbol=symbol, market="KRX", source="TEST", sequence_or_hash=f"risk-{symbol}-{sequence}",
        source_sequence=sequence, payload_hash="a" * 64, last_price=last_price,
        open_price=last_price, high_price=last_price, low_price=last_price,
        cumulative_volume=10000, best_bid_price=bid, best_bid_quantity=100,
...
def _position(db: Session, symbol: str, quantity: int, average_price: Decimal) -> Position:
    position = Position(
        account_alias=ACCOUNT_ALIAS, symbol=symbol, quantity=quantity,
        average_price=average_price, state="OPEN" if quantity > 0 else "CLOSED",
    )
    db.add(position)
    db.flush()
    return position
...
def test_daily_realized_loss_from_sells(db: Session) -> None:
    ...
    intent = OrderIntent(account_alias=ACCOUNT_ALIAS, environment="MOCK", symbol="005930", market="KRX", side="SELL", action="FULL_SELL", requested_quantity=10, correlation_id="c")
    db.add(intent)
    db.flush()
    order = TradingOrder(
        intent_id=intent.id, order_group_id=intent.order_group_id, account_alias=ACCOUNT_ALIAS,
        environment="MOCK", symbol="005930", market="KRX", side="SELL", order_type="LIMIT",
        requested_quantity=10, filled_quantity=10, remaining_quantity=0, status="FILLED", idempotency_key="k",
        request_hash="h", trading_date=NOW.astimezone(ZoneInfo("Asia/Seoul")).date(), correlation_id="c",
    )
    db.add(order)
    db.flush()
    db.add(Fill(order_id=order.id, broker_fill_key="f1", quantity=10, price=Decimal(49000), filled_at=NOW))
    db.commit()
...
def test_daily_loss_pct_realized_plus_unrealized(db: Session) -> None:
    ...
    pct = daily_loss_pct(db, ACCOUNT_ALIAS, basis="REALIZED_PLUS_UNREALIZED", now=NOW, denominator=Decimal(100000))
...
def test_daily_loss_pct_realized_only(db: Session) -> None:
    ...
    db.commit()
    # no realized sells -> 0%
    pct = daily_loss_pct(db, ACCOUNT_ALIAS, basis="REALIZED_ONLY", now=NOW, denominator=Decimal(100000))
...
def test_daily_entry_count(db: Session) -> None:
    intent = OrderIntent(account_alias=ACCOUNT_ALIAS, environment="MOCK", symbol="005930", market="KRX", side="BUY", action="BUY", requested_quantity=1, correlation_id="c")
    db.add(intent)
    db.flush()
    db.add(TradingOrder(
        intent_id=intent.id, order_group_id=intent.order_group_id, account_alias=ACCOUNT_ALIAS,
        environment="MOCK", symbol="005930", market="KRX", side="BUY", order_type="LIMIT",
        requested_quantity=1, remaining_quantity=1, status="CREATED", idempotency_key="k",
        request_hash="h", trading_date=NOW.astimezone(ZoneInfo("Asia/Seoul")).date(), correlation_id="c",
    ))
    db.commit()
...
def test_consecutive_loss_count(db: Session) -> None:
    ...
    for i, price in enumerate((Decimal(50500), Decimal(48500), Decimal(49000))):
        intent = OrderIntent(account_alias=ACCOUNT_ALIAS, environment="MOCK", symbol="005930", market="KRX", side="SELL", action="FULL_SELL", requested_quantity=1, correlation_id=f"c{i}")
        db.add(intent)
        db.flush()
        order = TradingOrder(
            intent_id=intent.id, order_group_id=intent.order_group_id, account_alias=ACCOUNT_ALIAS,
            environment="MOCK", symbol="005930", market="KRX", side="SELL", order_type="LIMIT",
            requested_quantity=1, filled_quantity=1, remaining_quantity=0, status="FILLED", idempotency_key=f"k{i}",
            request_hash="h", trading_date=NOW.astimezone(ZoneInfo("Asia/Seoul")).date(), correlation_id=f"c{i}",
        )
        db.add(order)
        db.flush()
        # i=0 is most recent (largest filled_at), i=2 is oldest.
        db.add(Fill(order_id=order.id, broker_fill_key=f"f{i}", quantity=1, price=price, filled_at=NOW - timedelta(hours=i)))
    db.commit()
...
def test_broker_connection_blocked_when_reconciling(db: Session) -> None:
    gate = TradingGate(account_alias=ACCOUNT_ALIAS, environment="MOCK", status="RECONCILING", reason="TEST", version=1)
    db.add(gate)
    worker = BrokerWorkerState(
        account_alias=ACCOUNT_ALIAS, environment="MOCK", state="READY", fencing_token=1,
        websocket_connected=True, subscriptions_ready=True, last_heartbeat_at=NOW, started_at=NOW,
