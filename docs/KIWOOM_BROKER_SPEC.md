...
- `kt10003` 취소는 원주문 번호와 현재 잔량으로 한 번만 호출하며 정상 응답도 실제 취소 완료가 아닌 접수로 처리한다.
- 취소 요청의 전송 결과가 불명확하면 동일 요청을 반복하지 않고 계좌 전체 재동기화로 전환한다.
...
`KIW-DEV-001`: 현재 개발 단계의 MOCK 1주 시험은 로그인 세션·CSRF·고정 확인문구·READY gate·요청 멱등성을 검증하되 별도 TOTP 재인증은 요구하지 않는다. KIW-137의 재인증 부분은 서비스 완성 후 재평가 전까지 보류한다.
