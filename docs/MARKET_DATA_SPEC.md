...
| MKT-082 | 단일 Broker worker는 계좌 이벤트 그룹과 분리된 그룹으로 활성 감시 종목의 KRX 코드(`005930`)와 NXT 코드(`005930_NX`)에 `0B`·`0D`를 등록한다. 시작·재연결 때 전체 목록을 재등록하고 실행 중에는 5초 이내 변경을 반영한다. 내부 저장 종목코드는 두 시장 모두 숫자 6자리로 정규화한다. |
...
| MKT-088 | wire item의 suffix가 없으면 `KRX`, `_NX`이면 `NXT`로 정규화한다. `_AL` SOR stream은 주문·시세 의미 검증 전 구독하지 않는다. 등록하지 않은 item과 알 수 없는 suffix는 폐기한다. |
| MKT-089 | 정상 NXT 시세를 한 번 이상 수신하면 종목별 venue 상태를 `VERIFIED/QUOTE_OBSERVED`로 영속화한다. NXT 시세 부재·지연·구독 전 상태는 `UNKNOWN`이며 이를 `INELIGIBLE`로 승격하지 않는다. `INELIGIBLE`은 향후 권위 있는 종목 목록 또는 명시적 Broker 응답으로만 기록한다. |
| MKT-112 | 키움 모의투자의 주문·계좌 REST 범위는 KRX로 제한되지만 NXT 시세 수신은 분석용으로 허용한다. NXT snapshot과 적격 상태는 MOCK 주문 송신 권한을 만들지 않는다. |

### 3.10 국내주식 거래일 캘린더

| ID | 요구사항 |
| --- | --- |
| MKT-108 | KRX와 NXT 세션 판정은 서버의 공통 `krx-calendar-v1` 거래일 정책을 사용한다. 브라우저·LLM·수신 시세 유무로 휴장 여부를 추정하지 않는다. |
| MKT-109 | 대한민국 공휴일·대체공휴일, 토·일요일, 근로자의 날과 KRX 연말 휴장을 닫힌 거래일로 처리한다. |
| MKT-110 | 캘린더 판정에는 정책 버전과 사유를 부여하고 venue 선택 등 거래 경계의 입력 hash와 감사 이력에 포함한다. 판정 실패는 fail-closed다. |
| MKT-111 | 임시 휴장과 개장시간 변경처럼 연간 규칙만으로 알 수 없는 일정은 후속 운영 override에 반영하며, override가 없는 상태에서 임의로 정상 개장으로 보정하지 않는다. |

기본 규칙은 KRX가 공개한 국내주식 휴장 기준(공휴일, 근로자의 날, 토요일, 연말 휴장)을 따르며 대한민국 공휴일·대체공휴일 산출은 버전이 고정된 `holidays` 패키지를 사용한다. 임시 시장조치까지 공식 일정 동기화가 완료된 것으로 간주하지 않는다.

- KRX holiday rule: <https://global.krx.co.kr/contents/GLB/06/0602/0602010201/GLB0602010201T1.jsp>
