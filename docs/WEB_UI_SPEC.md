...
| UI-149 | 판단과 Agent run 이력은 종목·시장·분석 맥락·행동 또는 SHADOW 평가·상태·시각·주요 reason 최대 3개만 포함한 요약 행으로 표시한다. DAG stage, 전체 reason, provider 호출과 구조화 응답은 사용자가 요약 행을 선택했을 때 하나의 상세 영역에서만 표시한다. |
