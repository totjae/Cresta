...
| UI-PAP-004 | 보유 포지션 화면은 legacy Paper 원장과 키움 MOCK account projection에 저장된 수량·평균단가·상태·version·관리 출처를 표시한다. `EXTERNAL`은 `키움 외부 보유`로 구분하며 현재가와 손익 데이터가 없으면 계산하거나 추정하지 않는다. |
