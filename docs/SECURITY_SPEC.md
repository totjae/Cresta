...
| SEC-091 | NAVER API HUB Client ID·Secret은 두 Docker secret 파일로만 API·Agent에 주입하고 인증 header, DB, stage 출력, evidence facts·hash와 오류에 포함하지 않는다. redirect를 금지하고 공식 HTTPS host·뉴스 path·응답 크기 상한을 고정한다. |
